...
class _SellTicket extends State<SellTicket> {
  ...
  void oneButtonDialog(String msg) { // 확인 버튼만 있는 다이얼로그
    showDialog(
        context: context,
  ...
  void selectionDialog() { // 취소, 확인 버튼이 있는 다이얼로그
    showDialog(
        context: context,
        barrierDismissible: true,
        builder: (BuildContext context) {
  ...
  Future<void> checkValidUploadFile() async { // 티켓 업로드 이전에 모든 입력된 정보가 유효할 지 확인
    if (name != "") {
  ...
  Future<void> uploadFile() async { // 티켓 업로드
    var price = int.tryParse(inputPriceController.text);

    String fileName = ticketImage!.path.split('/').last;
    final path = 'images/' + fileName;

    Map<String, dynamic> assetUploadRes = await assetUpload(ticketImage!.path!); // 에셋 업로드 진행

    if (assetUploadRes["statusCode"] == 200) { // 에셋 업로드 완료
      Map<String, dynamic> kip17res = await kip17TokenMinting(_selected, testAddress1, assetUploadRes["uri"]); // 토큰 생성

      if (kip17res["statusCode"] == 200) { // 토큰 생성 성공
