...
module.exports = {
    ...
    getBidStatus: function (token_id, callback) {
        const sql = "\
        SELECT nickname, bidder, bid_date, bid_price \
    ...
    get5DeadlineAuction: function (callback) {
        const sql = "\
        SELECT T1.token_id, T1.product_name, T1.owner, T1.place, T1.performance_date, T1.seat_class, T1.seat_No, T1.auction_end_date, T2.poster_url \
        FROM (SELECT token_id, product_name, owner, place, performance_date, seat_class, seat_No, auction_end_date \
        FROM user_db.auction_tickets NATURAL JOIN user_db.tickets \
    ...
    delAuctionData: function (token_id, callback) {
        const sql = "DELETE FROM user_db.auction_tickets WHERE token_id = (?);";
        connection.query(sql, [token_id], callback);
    }
}